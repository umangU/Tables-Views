...
CREATE TABLE "editions" (
	"isbn" text NOT NULL,
	"book_id" integer REFERENCES "books" ("book_id") MATCH SIMPLE
      ON UPDATE CASCADE ON DELETE CASCADE,
	"edition" integer,
	"publisher_id" integer REFERENCES "publishers" ("publisher_id") MATCH SIMPLE
      ON UPDATE CASCADE ON DELETE CASCADE,
	"publication" date,
	"type" character(1),
...
CREATE TABLE "stock" (
	"isbn" text NOT NULL REFERENCES "editions" ("isbn") MATCH SIMPLE
      ON UPDATE CASCADE ON DELETE CASCADE,
	"cost" numeric(5,2),
	"retail" numeric(5,2),
	"stock" integer,
...
CREATE TABLE "customers" (
	...
	"last_name" text,
	"first_name" text,
	Constraint "customers_pkey" Primary Key ("c_id")
);
...
CREATE TABLE "shipments" (
	...
	"c_id" integer REFERENCES "customers" ("c_id") MATCH SIMPLE
      ON UPDATE CASCADE ON DELETE CASCADE,
	"isbn" text REFERENCES "editions" ("isbn") MATCH SIMPLE
      ON UPDATE CASCADE ON DELETE CASCADE,
